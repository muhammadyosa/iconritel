...
export default function Teams() {
  ...
  // === Rekonsiliasi Live vs History (window terpisah dengan filter jam/hari) ===
  ...
  const reconStats = useMemo(() => {
    ...
    reconLiveCreatedTickets.forEach(t => {
      const s = ensure(getKey((t as any).createdByUserId, t.createdByName), (t.createdByName || "Unknown").trim());
      s.liveCreated += 1;
    });
    reconLiveResolvedTickets.forEach(t => {
      const s = ensure(getKey((t as any).resolvedByUserId || (t as any).createdByUserId, (t as any).resolvedByName || t.createdByName), ((t as any).resolvedByName || t.createdByName || "Unknown").trim());
      s.liveResolved += 1;
    });
    return Object.entries(stats)
      .map(([userKey, s]) => ({
                ...
                <Card className="shadow-card overflow-hidden">
                  <CardHeader className="py-2.5 px-3 sm:px-4 border-b bg-accent/5">
                    <CardTitle className="flex items-center justify-between text-xs sm:text-sm">
                      ...
                      <Badge variant="secondary" className="text-[8px] sm:text-[9px]">{rankingCustomRange?.from ? (rankingCustomRange.to && rankingCustomRange.to.getTime() !== rankingCustomRange.from.getTime() ? `${format(rankingCustomRange.from, "dd MMM", { locale: localeId })} - ${format(rankingCustomRange.to, "dd MMM", { locale: localeId })}` : format(rankingCustomRange.from, "dd MMM yyyy", { locale: localeId })) : "7 Hari"}</Badge>
                    </CardTitle>
                  </CardHeader>
              ...
              <Dialog open={!!reconUserDetail} onOpenChange={(open) => !open && setReconUserDetail(null)}>
                <DialogContent className="max-w-3xl max-h-[85vh] overflow-hidden flex flex-col">
                  ...
                  {reconUserDetail && (() => {
                    ...
                    const liveCreated = reconLiveCreatedTickets.filter(t => matchUser((t as any).createdByUserId, t.createdByName));
                    const liveResolved = reconLiveResolvedTickets.filter(t => matchUser((t as any).resolvedByUserId || (t as any).createdByUserId, (t as any).resolvedByName || t.createdByName));
